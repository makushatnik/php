...
class CustomerAuditTest extends Unit
{
    /**
    ...
    {
        //Dependency
        $identity = UserRecord::findOne(['username' => 'admin']);
        $user = \Yii::$app->user;
        //Given
        $user->login($identity);
    ...
    public function CustomerRecordRemembersUpdateDatetimeAndUser() {
        //Dependency
        $first_identity = UserRecord::findOne(['username' => 'admin']);
        $second_identity = UserRecord::findOne(['username' => 'manager']);
        $user = \Yii::$app->user;
        //Given
    ...
    private function assertBetween($before, $time, $after) {
